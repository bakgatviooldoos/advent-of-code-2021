...
(define row-count (length octopus-energy-levels))
(define col-count (length (first octopus-energy-levels)))
...
(define (neighbourhood i j)
  (for/list ([oi '(+1 +1 +1 +0 -1 -1 -1 +0)]
             [oj '(-1 +0 +1 +1 +1 +0 -1 -1)]
             #:when (and (<= 0 (+ i oi) (sub1 row-count))
                         (<= 0 (+ j oj) (sub1 col-count))))
    (cons (+ i oi) (+ j oj))))

(define (adjacent-flashes energy-levels i j)
  (for/sum ([neighbour (neighbourhood i j)])
    (if (< 9 (get-level energy-levels (car neighbour) (cdr neighbour))) 1 0)))
...
(define (simulate-step energy-levels)
  ...
  (define part-1->2
    (let loop ([energy-levels part-1]
      ...
      (define flashcount 0)
      (define do-flashes
        (for/list ([i row-count])
          (for/list ([j col-count])
            (define level (get-level energy-levels i j))
            (cond [(< 9 level)
